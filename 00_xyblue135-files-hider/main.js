...
module.exports = class FolderHiderPlugin extends Plugin {
  constructor(...args) {
    super(...args);
  ...
  async saveHiddenFolders() {
    await this.saveData({ hiddenFolders: [...this.hiddenFolders].sort() });
  }
  ...
  patchRevealTarget(target) {
    if (!target || typeof target.revealInFolder !== "function") return;
    const current = target.revealInFolder;
  ...
  patchRevealInFolder() {
    ...
    for (const leaf of this.app.workspace.getLeavesOfType("file-explorer")) this.patchRevealTarget(leaf.view);
  }
